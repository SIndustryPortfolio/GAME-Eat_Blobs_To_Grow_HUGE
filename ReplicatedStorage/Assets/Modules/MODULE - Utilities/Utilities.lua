...
function UtilitiesModule.RenderSteppedFPSLock(NilParam, FPS, CallBackFunction, ...)
	...
	local function OnRenderStep(Time, Delta)	
		-- CORE
		DeltaTime = DeltaTime + Delta

		if DeltaTime < AverageDelta then
			return nil
		end
		...
		CallBackFunction(Time, unpack(Args))
	end

	Connection1 = RunService.Stepped:Connect(OnRenderStep) --RunService.RenderStepped:Connect(OnRenderStep)

	return Connection1
end


function UtilitiesModule.FormatCurrencyString(NilParam, Number)
	-- Functions
	-- INIT
	if Number > 1000 then
		local String = tostring(Number / 1000)
		local FoundDecimalPoint = string.find(String, ".")
		return string.sub(String, 1, FoundDecimalPoint + 2).. "K"
	else
		return Number
	end
end

function UtilitiesModule.CubeRoot(NilParam, Number)
	return (Number ^ (1 / 3))
end
...
function UtilitiesModule.ApplyProperties(NilParam, Element, Properties)
	...
	for PropertyName, PropertyValue in pairs(Properties) do
		...
		if not Success then
			local Success, Error = pcall(function()
				return Element:SetAttribute(PropertyName, PropertyValue())
			end)
			
			if not Success then
				print(script.Name.. " | ApplyProperties | Error: ".. tostring(Error))
			end
		end
	end
end
